...
**SR2. Three honest producers:** `Recorder::stage`'s three skip sites (over-cap pre-check, `PutResult::OverCap`, `PutResult::IoError`, plus `unreadable`/`fs::read` failure) each set the matching `skipped_reason`, asserted at the `stage`/`resolve` level — one test per cause, real filesystem fixtures (an actual >10 MiB file for `over_cap`, a chmod-0000 file for `unreadable`, a locked object-store parent dir for `io_failed`), not synthetic structs.
...
**SR4. Honest read-side vocabulary, one shared helper (`fmt::skip_reason_text`):** `diff`'s per-file notice reads "content not snapshotted (over size cap|write failed at record time|file unreadable at record time|reason unrecorded)" per the real cause; a present-but-unresolvable blob (hash recorded, `store.get` fails) reads "(snapshot unavailable)" with no asserted cause — the pre-D45 "purged or missing" suffix is gone because the true cause is genuinely unknown at that point. `undo`'s `StoreError::Missing` refusal on a `before` hash reads "prior snapshot unavailable — refusing to restore" (was conflated with the "before is absent entirely" case); the corrupt/hash-mismatch message is untouched.
