...
| D52 | *(Codex integration completion, 2026-09-06)* A resolved Stop declaration is consumed only by the final rich turn that Stop closes. The resolver prefers the signal's `files_written` and falls back to its named transcript. Observed declared paths persist `attribution: "declared"`; other observed paths persist `"undeclared"`; no resolved declaration leaves attribution absent. Declarations never create file entries, never rewrite an earlier blocked-continuation turn, and never affect undo safety. Codex start/stop signals carry a per-invocation `emitter_event`, so an exact replay can be deduplicated without swallowing a separately-fired same-turn continuation. `--no-hook` wins over `--codex` and skips both hooks and MCP registration. Malformed Codex hook configuration exits 1 before any subsequent Codex or service mutation. |
...
6. `--no-hook` overrides `--codex`: neither Codex hooks nor Codex MCP registration is written. Tests: `codex_init_no_hook_skips_hooks_and_mcp`.
7. Malformed `hooks.json` or `config.toml` makes `init --codex` exit 1, preserves the malformed file byte-for-byte, writes no backup, and does not continue into another Codex configuration surface. Tests: `codex_init_malformed_hooks_json_aborts_without_codex_side_effects`, `codex_init_malformed_config_toml_aborts_untouched`.
8. D6 per-file attribution uses the exact open-enum values `declared` / `undeclared` / absent, only on the Stop-closed turn; out-of-root declarations are omitted and reported. Every Codex start/stop invocation mints a distinct `emitter_event`; exact signal replays retain it, so blocked continuations remain distinct even with the same timestamp and path set. The deterministic persistence/dedup tests pass; the live watcher-backed test remains UNTESTED locally because the existing watcher does not stage ordinary writes in this environment. Tests: `d6_attr_persist_writes_exact_declared_and_undeclared_values`, `d6_attr_absent_declaration_stays_absent_on_every_file`, `d6_attr_stop_declaration_applies_only_to_final_closed_turn`, `handle_emitter_turn_signal_same_files_same_ms_new_event_is_a_continuation`, `d6_attr_stop_declaration_persists_declared_and_undeclared_files`.
...
never becomes `DECLARED`. Disclosed with it, and **asserted by the test** rather
than left in prose: the 59 rows cover **55 distinct (status, severity, stale)
cells**. Four pairs collapse — `declared × none × stale=true` folds like
`derived × none × stale=true` (it writes only the `Stale` event), and each of
`claim_false × {none, blocking, fyi} × stale=true` folds like its `stale=false`
