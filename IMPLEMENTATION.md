...
| D35 | *(hardening review 2026-07-09)* Snapshot-failure taxonomy: `BlobStore::put` returns a typed result distinguishing over-cap from I/O failure (disk full, EPERM). The wire format is unchanged — `skipped: true` covers both, no PROTOCOL change — but the *reason* is operational data: I/O failures increment a counter persisted in `state.json`, log to stderr at occurrence, and surface in `status` as a `DEGRADED — N snapshot writes failed` banner naming the likely cause. `undo` on an I/O-skipped file says "no snapshot exists (write failed at record time)", never "over size cap". A safety net that is silently absent is worse than none. |
