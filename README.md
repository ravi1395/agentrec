...
| `agentrec memories` | List recorded memories (audit view, not a query). `--stale` (non-fresh only), `--all` (include retracted), `--json`. `--stats` summarizes per-hook recall latency instead: reads `memory-stats.jsonl` directly (never through the `memory.jsonl` load path, so a corrupt memory store never blocks this readout) and reports count/p50/p90/p99/max over lines carrying `elapsed_ms`, a per-outcome breakdown (injected/budget_exceeded/failure/capped), plus honest counts of pre-upgrade lines (parseable, no `elapsed_ms` — written before this field existed) and unparseable lines skipped. An absent or empty stats file prints `no hook invocations recorded` |
