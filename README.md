...
- `--no-hook` is a global integration opt-out: it overrides `--codex` and installs neither agent hooks nor MCP registrations, leaving bare-turn-only capture.
...
- A malformed Codex hook config is left untouched and makes `init` exit 1 before Codex MCP registration or service installation. Base scaffolding and Claude Code setup run earlier and may already have completed.
...
- **Bracketing.** Claude Code and Codex use `UserPromptSubmit` to open a turn and `Stop` to close it. While the bracket is open, quiet-window closure is suppressed; on stop, interim bare turns are retroactively merged into the rich turn. Codex signals carry a per-hook-invocation identity, so an exact replay is deduplicated without collapsing a separately-fired blocked continuation. The recorder resolves the declared-write set from the closing signal's `files_written`, falling back to the named transcript when needed, then intersects it with observed filesystem activity: declared paths carry per-file `attribution: "declared"`, other observed paths carry `"undeclared"`, and no resolved declaration leaves attribution absent. A declaration never invents a file entry or proves that an undeclared write was human-authored.
...
`agentrec init` scaffolds `.agentrec/`, installs Claude Code hooks (`UserPromptSubmit` + `Stop`, merged additively into `.claude/settings.local.json` — existing third-party hooks on the same events are preserved), writes Claude Code's MCP registration, adds `.agentrec/` to `.gitignore`, tightens store permissions (0700/0600), and installs+loads a per-repo recorder service. `--codex` additionally installs Codex hooks (`UserPromptSubmit`, `PostToolUse` matched to `apply_patch`, and `Stop`) plus repo-local MCP configuration; Codex requires the trust step described in [Quickstart](#quickstart). `--no-hook` overrides `--codex` and skips all hook and MCP integration. Flags: `--codex`, `--no-hook`, `--no-service`, `--service`, `--dry-run`. **Under a temporary directory** (`$TMPDIR`, `/tmp`, `/private/tmp`) the service unit is skipped by default and the reason is printed: the unit is user-scoped with `RunAtLoad`+`KeepAlive`, so it outlives the directory it records and nothing reaps it when that directory is deleted. `--service` installs it anyway; `--service` together with `--no-service` is rejected.
...
Parser caveats worth knowing: the config is real TOML (`cli/src/config.rs`). Invalid TOML is a hard error, reported with line and column, **at daemon startup (`record`) and on the verbs that actually read the config** — `status` (store budget), `log` and `show` (`noise_globs`, read when there is something to render), `purge` (`ttl_days`), `mcp`, and `attest coverage` (`attest_coverage_path`, which must agree with the daemon). Verbs that never read it — `doctor`, `recall`, `remember`, `attest derive`, `attest status`, `attest verify` — run fine with a broken config, and the memory verbs read fail-open by design (invariant INV-M4: a hook always appends its signal). Because the error surfaces where the read happens, a verb can exit 0 on a trivial input and fail on a real one: `log` on a repo with no turns never consults `noise_globs`, so it succeeds; the same `log` on a repo with turns hard-errors (both measured). For the same reason `status`, `purge` and `mcp` refuse before printing anything, while `show` prints the turn header first and then exits 1. A missing file or missing key falls back to the default; a present key with the wrong value type falls back to that key's default, except `mcp_destructive`, whose invalid value is a hard error (it gates agent-driven undo).
