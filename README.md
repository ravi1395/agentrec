...
| `agentrec status` | Store size, recording gaps, rich-rate, DEGRADED banner on snapshot or prompt-blob write failures, and — only once one has happened — how many times the ignore rules were reloaded. `--ack-degraded` clears the banner; `--json` emits the ignore-reload counters instead of the text report (operational state, not the `log.jsonl` protocol, and it does **not** yet carry the DEGRADED fields) |
