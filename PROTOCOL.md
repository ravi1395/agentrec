# The agentrec protocol — 1.0 (frozen 2026-08-06)
...
**Frozen at 1.0 on 2026-08-06.** Both schemas this document defines — the signal schema (§4) and the record schema (§5) — are stable. Within major `v: 1`, every future change is **additive only, forever**: a field MAY be added; no field may be removed, renamed, retyped, or have its meaning narrowed, and no already-defined value of an existing field may change meaning. An open string enum (`link_kind`, `skipped_reason`, `attribution`, §5) MAY gain values, because consumers are already required to tolerate ones they do not recognize. Anything that cannot be expressed that way requires a new major, which is a new schema rather than an edit to this one (§10). **Freezing to 1.0 does not change any line's `v`.** The 1.0 versions this *document*; both schemas stay `v: 1`, and every record written before this date remains valid and readable. Machine-checkable examples of the frozen shapes — valid, tolerated, and refused — live in `docs/fixtures/conformance/`. *(Freeze mandated by spec decision 5 and delta decision 16; the epoch record's `dropped_signals` count (§5, D51) landed immediately before the freeze specifically so it is inside it.)*

*Document history — v0.2 incorporated the hostile-review fixes (REVIEW.md): start/stop bracketing with retroactive merge, epoch records, ULID ids, millisecond timestamps, the two-predicate change model, git-operation turns, and withheld snapshots. Published 2026-07-10 alongside M2 "Answer" + M3 "Ship": additive-only wire changes since the draft are the turn-record `merges` field (retroactive-merge bookkeeping, §5) and the file-entry `baseline_unknown` flag (§5). Snapshot-failure/DEGRADED state is implementation-local (`.agentrec/state.json`) and is deliberately NOT part of the wire protocol — see the note at the end of §5. A further additive field, the file-entry `skipped_reason` open enum (§5), landed pre-1.0-freeze to give `skipped: true` an honest per-cause explanation (`over_cap` / `io_failed` / `unreadable`, with `policy` reserved for a future producer) instead of consumers assuming a single cause. So did the epoch-record `dropped_signals` count (§5, D51), which makes an offline-dropped tool bracket visible instead of silent without replaying it.*
...
| `prompt` | string \| null | MAY | The user prompt for this turn. Scrub responsibility is split exactly as it is for `fact` below: an emitter SHOULD pre-scrub (both reference emitters do), and the **recorder MUST scrub before persistence** (§9) whether or not the emitter did — an emitter's scrub is a courtesy, never a guarantee a recorder may rely on. Carried on `"start"` in practice (that is where a prompt exists), but the field is **not restricted by `event`**: a recorder MUST accept it on a `"stop"` too, where it supplies the prompt for a stop-only emitter that never opened a bracket. A recorder SHOULD prefer it over any transcript-derived extraction (`transcript`, above) and MUST fall back to extraction when it is absent — the field is the emitter's own first-hand copy, extraction is a guess at which transcript entry was the prompt. Scrubbing an already-scrubbed prompt is idempotent, so the double pass costs nothing. Unlike the fields below, this one is written as an **explicit `null`** by the reference emitters rather than omitted; an explicit `null` is semantically identical to absence, and both mean "no prompt declared", never "empty prompt" |
...
| `emitter_turn` | string | MAY | On `"start"`/`"stop"` only: a stable turn identity the **emitter** assigns (e.g. Codex's own hook `turn_id`) — not a recorder-side id. A recorder MAY use it to correlate signals and MAY use a mismatch between a `stop`'s `emitter_turn` and an open bracket's own to recognize the `stop` as not closing that bracket. It is not sufficient replay evidence because a blocked continuation may legitimately fire multiple Stops for one turn. Absence means "emitter has no such id" (every Claude Code hook today), never "no upstream turn" — a recorder MUST fall back to ordinary tool-based start/stop matching whenever either side of a comparison lacks it. Two signals both lacking `emitter_turn` are never considered a match on that basis |
| `emitter_event` | string | MAY | On `"start"`/`"stop"` only: a stable identity for this individual signal emission. Each separately-fired hook invocation MUST mint a new value, including blocked-continuation Stops that share an `emitter_turn`; an exact replay MUST retain the original value. A recorder MAY use it to distinguish a replay from another firing without relying on timestamp resolution or mutable payload content. Absence means the emitter cannot make that distinction; a recorder MUST NOT assume two same-turn signals are duplicates solely because their remaining fields coincide |
...
| `imported` | bool | MAY | `true` marks a turn **reconstructed from a foreign tool's own transcript** by a bulk importer, rather than observed by a running recorder. Its boundaries, prompt and file list come from re-reading history; no hook ever fired and no filesystem event was ever watched for it. Omitted (never `false`) on every live-recorded turn, so a recorder that does not import writes byte-identically to a pre-this-field one. Consumers MUST NOT treat an imported turn as evidence that emitter integration is working — the reference `status` excludes imported turns from its rich-rate window entirely, precisely so a bulk import cannot make a dead hook read as healthy — and MUST NOT revert a file entry on an imported turn whose `before` is `null` on a non-`create` op: that `null` is a failed reconstruction, not a created file, and there are no pre-edit bytes to restore. The reference implementation refuses the **whole** undo in that case rather than reverting the turn's other entries, so a partial revert can never be mistaken for a complete one. See also `after_synthesized` (below), which marks the converse hazard — an `after` that is derived rather than observed |
| `files_complete` | bool | MAY | Whether `files` is an attested-complete accounting of what the turn touched. Written as `false` **unconditionally on every imported turn** by this implementation, and never written at all otherwise; `true` is defined but has no producer — it is reserved for a future importer or recorder that can attest completeness. It therefore discriminates *"imported, so partial"* and **nothing finer**: a `false` says only that entries may be missing (a source transcript records opaque tool calls and non-file activity the importer cannot turn into file entries), never that entries were in fact dropped from this particular turn, and never how many. Consumers MUST NOT read it as a per-turn drop count or as a data-loss marker. Absence means "not declared", never "complete" — every pre-this-field record and every live-recorded turn omits it |
| `origin` | string | MAY | Additive (F5). Which surface **executed** an undo turn's writes: `"cli"` (a human ran `undo --confirm` or `approve`) or `"mcp"` (an agent spent an auto-mode token, §8). Meaningful on undo turns only — the reference implementation writes it exactly where it writes `tool: "agentrec"` and omits it everywhere else, so every non-undo record is byte-identical to a pre-this-field one. **Absent means `"cli"`**, not "unknown": every undo turn written before this field existed was one, and consumers MUST apply that default rather than treating absence as a third state. It names the *executing* surface and never the *requesting* one — an `approve` of an undo an agent requested over MCP is `"cli"`, because a human performed it; which request it settled is recorded in the implementation-local request ledger, not here. A consumer counting agent-initiated reverts MUST therefore count `"mcp"` only, and MUST NOT read `"cli"` as "no agent was involved". Consumers MUST tolerate an unrecognized value (§10) and MUST NOT fold it into either defined value |
| `sig` | object | MAY | Reserved for signed entries (future minor version) |

File entry: `{ "path": "src/auth.ts", "before": "sha256:...|null", "after": "sha256:...|null", "op": "create|modify|delete", "skipped": true?, "skipped_reason": "over_cap|io_failed|unreadable|policy"?, "withheld": true?, "baseline_unknown": true?, "after_synthesized": true?, "link_kind": "symlink"?, "attribution": "<producer-defined>"? }` — `before: null` means created, `after: null` means deleted, `skipped: true` means the file's content was NOT captured to the object store — this can happen for more than one reason (below) — `withheld: true` means content matched the secret-file denylist and was deliberately not snapshotted (§9). Both flagged kinds record the boundary only and are never revertible. `baseline_unknown: true` marks a `modify` entry whose `before` is `null` not because the file was created, but because the recorder first observed it mid-session (e.g. daemon started after the file already existed and was then touched) — the true prior content is unrecoverable; consumers MUST render this distinctly from a normal create and MUST NOT infer the file was new.
...
**`after_synthesized` (additive, v1.0+).** `true` marks an entry whose `after` hash addresses **derived** bytes — content the producer computed (e.g. an imported turn's recorded `oldString`→`newString` substitution applied to a `before`) rather than content it observed, from a live snapshot or from a full-file body the source itself recorded. Omitted (never `false`) on every entry whose `after` is observed, which is every live-recorded entry, so the wire shape of those is byte-identical to a pre-this-field one. Absence therefore means "observed, or unknown", never "proven observed". It is only meaningful alongside an `after` that is actually present: an entry with `after: null`, including a `skipped` entry whose refs were nulled, carries no `after` to be misread and omits this flag. Consumers MUST NOT present a derived `after` as recorded fact, and specifically MUST NOT attribute a mismatch between the live file and a synthesized `after` to a human or external edit — the comparison point was never observed, so the mismatch attributes to nothing. This refusal outranks any other attribution signal a consumer holds (a later rich turn, a recording gap): those are real facts about *observed* history, and none of them makes an unobserved baseline trustworthy. Diff-style renderers SHOULD say the after-state is derived rather than showing it bare.
...
Rails that hold in every mode: files with `skipped: true` or `withheld: true` are never revertible (no snapshot content exists). Files failing the **modified-since** predicate check (§5 — content changed since the turn, by anyone or anything) are excluded unless the call sets `allow_modified: true` — which is honored **only** through explicit human approval in `confirm` mode. In `auto` mode a call setting `allow_modified: true` MUST be **refused outright**, not honored and not silently ignored: an agent MUST NOT be able to lower this rail for itself, and a token-bound auto grant is still the agent granting itself. Undo safety never keys on the weaker human-edited-since predicate.
...
`v` is per-line, per-schema. Within a major version, changes are additive only; consumers MUST ignore unknown fields and MUST NOT fail on them. A major bump is a new schema, and writers MUST NOT mix majors within one file. This document is at **1.0 and frozen** (see the freeze clause at the top): both schemas it defines are `v: 1`, they stay `v: 1`, and additive-only is now a permanent constraint rather than a convention pending a gate. The memory-candidate signal (§4) is an additive variant of the existing signal schema — it does not bump `v`.
