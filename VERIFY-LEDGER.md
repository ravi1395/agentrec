...
_(see the Open table near the top.)_ **J3 (CI matrix) and I++1 (Linux perms) closed this round** — repo published at https://github.com/ravi1395/agentrec, Actions matrix green. Remaining un-run items: the 7-night D36 streak (nightly workflow now live; needs the clock), Ubuntu-container/clean-macOS-VM/brew installer legs, Linux **inotify** induced-low-watches mode (Y++2 — the normal path runs green on Linux CI but no step lowers `fs.inotify.max_user_watches` to hit the fail branch yet), and the README blame GIF.
